...
def test_flags_future_annotations_import() -> None:
    """A __future__ annotations import is CAW001."""
    assert "CAW001" in _codes("from __future__ import annotations\n")


def test_flags_bare_basemodel_subclass() -> None:
    """A model inheriting BaseModel directly is CAW002."""
    source = "class Thing(BaseModel):\n    pass\n"

    assert "CAW002" in _codes(source)
...
def test_flags_ellipsis_in_protocol_method() -> None:
    """An `...` body in a Protocol method is CAW003."""
    source = (
        "class P(Protocol):\n"
        "    def f(self) -> int:\n"
        '        """Do f."""\n'
    ...
    assert "CAW003" in _codes(source)


def test_flags_double_backticks_in_docstring() -> None:
    """Double backticks in a docstring are CAW004."""
    source = '"""See ``graph.toml`` for details."""\n'

    assert "CAW004" in _codes(source)


def test_flags_homogeneous_tuple_annotation() -> None:
    """A `tuple[T, ...]` annotation is CAW005."""
    source = "values: tuple[int, ...] = ()\n"

    assert "CAW005" in _codes(source)


def test_flags_string_forward_ref_return() -> None:
    """A string return forward-ref to the enclosing class is CAW006."""
    source = (
        "class Thing:\n"
        "    @classmethod\n"
        '    def make(cls) -> "Thing":\n'
    ...
    assert "CAW006" in _codes(source)
