...
Encodes the rules in the `repo-coding-conventions` policy:
  ...
  CAW006  return `Self`, never a string forward-ref to the enclosing class

Run: `uv run python -m tools.repolint [paths...]` (defaults to the package sources).
Pass `--fix` to auto-apply the safe textual fixes (CAW001, CAW004).
"""
...
DEFAULT_PATHS = ("backend", "frontend-tui")
DOUBLE_BACKTICK = "`" * 2
...
def fix_source(source: str) -> str:
    """Apply the safe textual fixes (CAW004 single backticks, CAW001 future import)."""
